...
public class Practicaja extends Atm implements IOperacionesBasicas, IOperacionesAvanzadas {
    ...
    public Object[] retirar(CuentaDTO cuenta, double monto)
        ...
        if(monto%100!=0){
            throw new InvlidAmountException(Mensajes.INVALID_WITHDRAWAL_AMOUNT);
                ...
                LocalDate.now()).size() == Constants.DAILY_MAX_QUANTITY_WITHDRAWAL_LIMIT) {
    ...
    public Ticket depositar(CuentaDTO cuenta, double monto) throws BusinessException{
        ...
        if(monto%50!=0 && monto%20!=0){
            ...
            Atm.cuentas.set(indexAnterior,cuenta);
            this.cuentaDAO.actualizarSaldo(cuenta.getNumCuenta(),cuenta.getSaldo());
            MovimientoDTO mov = new MovimientoDTO(cuenta.getCuentaId(),"DEPOSITO", Date.valueOf(LocalDate.now()),
                    monto);
            this.movDAO.registrarMovimiento(mov);
            ticket = new Ticket(this.getUbicacion(),
                    Atm.folio++,
