...
//Manejador global de excepciones: Maneja la propagación de cualquier excepción lanzada en cualquier punto del sistema
//(de este servicio).
//Maneja excepciones genéricas (qu vienen de ResponseEntityExceptionHandler), lo sé por el @Override
//Maneja excepciones personalizadas (específicas del negocio), lo sé por el @ExceptionHandler
...
//    @ExceptionHandler(AccountWithAlreadyRegisteredCard.class)
//    public ResponseEntity<Object> handleAccountWithAlreadyRegisteredCardException(RuntimeException ex) {
//        ApiErrors errores = new ApiErrors(ex.getMessage(), HttpStatusCode.valueOf(409), LocalDateTime.now());
//        return ResponseEntity.status(HttpStatusCode.valueOf(409)).body(errores);
//    }
}
