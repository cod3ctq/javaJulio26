...
public interface CuentaRepository extends JpaRepository<Cuenta,Integer> {
    //Aqui ya se tiene toda la logica de consultas
    // (INSERT, SELECT, UPDATE, DELETE)
    //prefabricada, al menos las consultas basicas
    //Cuando se requiera, aqui mismo se agregan las consultas mas complejas
    //usando JPQL
    //JPQL: Java Persistence Query Languaje - Consultas Orientas a Objetos

    //UPDATE Cuenta c SET c.status =: 0 WHERE c.numCuenta =: 923392479
    //=: sea igual a
    //@Param describe los argumentos
    @Transactional //indica que si falla la consulta, revierta los cambios en automatico
    @Modifying //cualquier consulta que no usa un SELECT, requiere esta anotacion
