...
//*************MANEJADOR GLOBAL DE EXCEPCIONES: Maneja la propagacion de cualquier excepcion lanzada en
//cualquier punto del sistema (de este servicio).
// MANEJA EXCEPCIONES GENERICAS: QUE VIENEN DE RESPONSEENTITYEXCEPTIONHANDLER, TIENEN @OVERRIDE
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    @ExceptionHandler(AlreadyExistSameAccountException.class)
    ...
    public ResponseEntity<Object> handlerNotExistException(RuntimeException ex) {
        ApiErrors errores = new ApiErrors(ex.getMessage(), HttpStatusCode.valueOf(204));
        System.out.println(errores);
        return ResponseEntity.status(HttpStatusCode.valueOf(204)).body(errores);
    }

    @ExceptionHandler(BussinessException.class)
    public ResponseEntity<Object> BussinessException(RuntimeException ex) {
        ApiErrors errores = new ApiErrors(ex.getMessage(), HttpStatusCode.valueOf(422));
        return ResponseEntity.status(HttpStatusCode.valueOf(422)).body(errores);
    }
